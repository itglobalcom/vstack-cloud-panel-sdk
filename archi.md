...
| --- | --- | --- | --- | --- |
| `public-api` — REST монолита | API | in | cloudmng | consumer: SDK следует за publisher'ом и своего контракта не определяет; расхождение имени/типа поля правится в SDK |
...
| `PUT /vmware/networks/{id}/edge/bandwidth` контракта не обёрнут, хотя маршрут опубликован | единственная операция в объёме раздела, у которой нет метода; паритет с контрактом неполный | не отклонение SDK, а обход дефекта publisher'а: endpoint рапортует успех, ничего не сохраняя (замерено на проде — задача завершилась `Completed`, сеть продолжила отдавать прежний `bandwidth_mbps`). Пропускная способность правится через `EditVmwareNetwork`; пересмотреть, когда publisher починит маршрут |
